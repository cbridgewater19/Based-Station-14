...
public abstract class SharedJobSystem : EntitySystem
{
    ...
    // Goobstation Change: Returns the amount of Pennies a player will receive when they finish a round as this job.
    public int GetJobPennies(ICommonSession player)
    {
        if (_playerSystem.ContentData(player) is not { Mind: { } mindId }
            || !MindTryGetJob(mindId, out var prototype))
            return 1;

        return prototype.Pennies;
    }
}