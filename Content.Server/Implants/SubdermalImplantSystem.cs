...
// SPDX-FileCopyrightText: 2023 Toms Alves <tomasalves35@gmail.com>
...
public sealed class SubdermalImplantSystem : SharedSubdermalImplantSystem
{
    ...
    {
        // Handle DeathAcidifierImplant specifically
        if (HasComp<DeathAcidifierImplantComponent>(uid))
        {
            if (component.ImplantedEntity != null)
            {
                var user = component.ImplantedEntity.Value;
                ...
                // Spawn the acidifier at the user's location
                Spawn("Acidifier", coords);

                // Gib the user and delete their items
                if (TryComp<GibOnTriggerComponent>(uid, out var gibComp))
                {
                    _body.GibBody(user, gibOrgans: false, launchGibs: true, splatModifier: 1f);
                }
            }
        }
